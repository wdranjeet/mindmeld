...
const mockBhaktiSongs: Song[] = [
  ...
  },
  {
    ...
    url: 'https://sample-videos.com/zip/10/mp3/mp3-sample.mp3',
    duration: '7:45',
...
export const searchSongs = async (query: string): Promise<SearchResult> => {
  try {
    ...
    const songs: Song[] = response.data.data.results.map((item: JioSaavnSong) => ({
      id: item.id,
      title: item.name || item.title || '',
      subtitle: item.primaryArtists || item.subtitle || '',
      image: item.image?.[2]?.link || '',
      url: item.downloadUrl?.[4]?.link || item.url || '',
...
export const getTrendingSongs = async (): Promise<Song[]> => {
  try {
    ...
    return response.data.data.results.map((item: JioSaavnSong) => ({
      id: item.id,
      title: item.name || item.title || '',
      subtitle: item.primaryArtists || item.subtitle || '',
      image: item.image?.[2]?.link || '',
