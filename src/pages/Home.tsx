...
import { FiSearch, FiPlay, FiHeart, FiMusic, FiWifi, FiUser } from 'react-icons/fi'
...
function Home() {
  ...
  useEffect(() => {
    ...
    const loadInitialSongs = async () => {
      try {
        ...
        // Get custom uploaded songs
        const customSongsStr = localStorage.getItem('customSongs')
        const customSongs: Song[] = customSongsStr ? JSON.parse(customSongsStr) : []
        
        // Combine custom songs with trending songs
        const allSongs = [...customSongs, ...trendingSongs]
        setSongs(allSongs)
      } catch (error) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Filter custom songs by search query
      const filteredCustomSongs = customSongs.filter(song =>
        song.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (song.artist && song.artist.toLowerCase().includes(searchQuery.toLowerCase()))
      )
      
      // Combine filtered custom songs with search results
      const allSongs = [...filteredCustomSongs, ...result.songs]
      setSongs(allSongs)
    } catch (error) {
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-full hover:shadow-lg transition-all"
              >
                <FiUser />
                <span>Admin</span>
              </button>
